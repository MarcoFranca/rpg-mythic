...
type Props = { onOpen: (section: MenuId) => void; onDingPlay?: () => void };

export function PlayerObelisk({ onOpen, onDingPlay }: Props) {
    const { theme } = useEter();
    ...
    // ===== “lampada com defeito” — keyframes de flicker (opacidade/brightness)
    const flicker: TargetAndTransition = reduce
        ? { opacity: 1, filter: "brightness(1)" }
        : {
            opacity: [1, 1, 0.25, 1, 0.15, 1, 1, 1, 0.4, 1, 1],
            filter: [
                "brightness(1)",
                "brightness(1.1)",
                "brightness(0.7)",
            ...
            transition: {
                ...
                // mantém o “ritmo” das piscadas
                times: [0.01, 0.04, 0.08, 0.12, 0.16, 0.2, 0.6, 0.75, 0.8, 0.9, 1],
            },
        };

    // ===== áudio “ding” na 1ª interação
    const maybePlay = () => {
        if (!audioEnabled) return;
        if (onDingPlay) return onDingPlay();
        try { const a = new Audio("/sfx/ding.mp3"); a.volume = 0.45; void a.play(); } catch {}
    };
        ...
        <div ref={rootRef} className="relative grid place-items-center">
            ...
            <svg width="0" height="0" aria-hidden="true" className="absolute">
                <defs>
                    ...
                    {/* Ruído espectral (overlay bem sutil) */}
            ...
            <motion.button
                ...
                onClick={() => { setOpen(v => !v); setAudioEnabled(true); maybePlay(); }}
                onMouseMove={onMove}
                onMouseLeave={onLeave}
                className="relative grid h-44 w-44 cursor-pointer place-items-center rounded-full focus:outline-none focus-visible:ring-2"
                style={{
            ...
            >
                ...
                <motion.div
                    ...
                    transition={{ duration: 7, repeat: reduce ? 0 : Infinity, ease: "easeInOut" }}
                    style={{
                        // @ts-ignore
                ...
                >
                    {/* PRISMA — aplica flicker + vidro + ruído */}
                    <motion.div
                        animate={flicker}
                        ...
                        style={{
                            opacity: 0.96,
                            mixBlendMode: "screen",
                            filter: "url(#ether-glass)"
                        }}
                    >
                        <PrismaSVG className="h-28 w-28" style={{ color: theme.accent }} />
                        {/* ruído espectral sutil apenas dentro */}
                        {!reduce && (
                            <div className="pointer-events-none absolute inset-0" style={{ filter: "url(#ether-noise)" }} />
                        )}
                    </motion.div>
                    ...
                    {/* === FEIXE DE LUZ VOLUMÉTRICO (substitui o conic anterior) === */}
                    {!reduce && (
                        <>
                            {/* Shaft: raio “gordo” com máscara circular e blur, passando por trás do prisma */}
                            <motion.div
                                aria-hidden
                                ...
                                style={{
                                    width: 240,
                                    height: 240,
                                    // máscara radial com bordas suaves (sem quadrado)
                                    ...
                                    // volume do feixe (vertical, do alto para baixo)
                                    background:
                            ...
                            {/* Glint: risca fina cortando o prisma (parece reflexo no cristal) */}

                        </>
                    )}


                    {/* CÁUSTICAS (pontinhos de luz orbitando) */}
                    {!reduce && (
                        <>
                            {[
                                ...
                                >
                                    {/* posiciona o ponto no raio usando translate */}
                ...
                {/* glow respirando ao redor (sincroniza levemente com flicker) */}
                {!reduce && (
                    <motion.div
            ...
            {/* Menu radial */}
            <div id={menuId} className="absolute inset-0 z-20" style={{ pointerEvents: open ? "auto" : "none" }} aria-hidden={!open}>
                {items.map((it, idx) => {
